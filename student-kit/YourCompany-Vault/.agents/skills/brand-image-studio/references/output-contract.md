...
只有用戶要求 final、正式上架、客戶交付或精準平台尺寸時，才為每張圖建立以下 `delivery.json` 並跑完整驗收。普通草稿不建立逐張 request／delivery／hash；在現有計劃的輸出清單保留路徑、來源、狀態及例外，已有 batch manifest 可沿用。Final QA 不改變 SKILL.md 的確認關卡。
